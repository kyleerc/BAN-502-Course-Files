...
This dataset (as is common with medical and healthcare datasets) requires quite a bit of cleaning and preparation before analysis. I’ll walk you through the cleaning before we start our random forest work.
...
```{r}
set.seed(1234)
datasplit = initial_split(drug_clean, prop=.7, strata = Nicotine)
trainingdrug = training(datasplit)
testingdrug = testing(datasplit)
nrow(trainingdrug)
...
Answer: True, but barely

```{r}
#Batch 1
age = ggplot(drug_clean, aes(x=Age, fill=Nicotine)) + geom_bar(position = "fill") + labs(title = "Age and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

gender = ggplot(drug_clean, aes(x=Gender, fill=Nicotine)) + geom_bar(position = "fill") + labs(title = "Gender and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

education = ggplot(drug_clean, aes(x=Education, fill=Nicotine)) + geom_bar(position = "fill") + labs(title = "Education and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

country = ggplot(drug_clean, aes(x=Country, fill=Nicotine)) + geom_bar(position = "fill") + labs(title = "Country and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

grid.arrange(age, gender, education, country)



#Batch 2
ethnicity = ggplot(drug_clean, aes(x=Ethnicity, fill=Nicotine)) + geom_bar(position = "fill") + labs(title = "Ethnicity and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

nscore = ggplot(drug_clean, aes(y=Nscore, x=Nicotine)) + geom_boxplot() + labs(title = "Nscore and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

escore = ggplot(drug_clean, aes(y=Escore, x=Nicotine)) + geom_boxplot() + labs(title = "Escore and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

oscore = ggplot(drug_clean, aes(y=Oscore, x=Nicotine)) + geom_boxplot() + labs(title = "Oscore and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

grid.arrange(ethnicity, nscore, escore, oscore)



#Batch 3
ascore = ggplot(drug_clean, aes(y=Ascore, x=Nicotine)) + geom_boxplot() + labs(title = "Ascore and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

cscore = ggplot(drug_clean, aes(y=Cscore, x=Nicotine)) + geom_boxplot() + labs(title = "Cscore and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

impulsive = ggplot(drug_clean, aes(y=Impulsive, x=Nicotine)) + geom_boxplot() + labs(title = "Impulsive and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

ss = ggplot(drug_clean, aes(y=SS, x=Nicotine)) + geom_boxplot() + labs(title = "SS and Nicotine Use") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

grid.arrange(ascore, cscore, impulsive, ss)
...
```{r}
set.seed(123)
folds = vfold_cv(trainingdrug, v=5)

recipedrug = recipe(Nicotine ~., trainingdrug) %>%
  step_dummy(all_nominal(),-all_outcomes())

modeldrug = rand_forest(mtry = tune(), min_n = tune(), trees = 100) %>%
  set_engine("ranger", importance = "permutation") %>% 
  set_mode("classification")

workflowdrug = workflow() %>% 
  add_model(modeldrug) %>% 
  add_recipe(recipedrug)

griddrug = grid_regular(mtry(range = c(2, 8)), min_n(range = c(5, 20)),levels = 10)

set.seed(123) 
tuned_residuals = tune_grid(
  workflowdrug, 
  resamples = folds,
  grid = griddrug)
...
```{r}
best_parameter = select_best(tuned_residuals, "accuracy")


final_workflow = finalize_workflow(workflowdrug, best_parameter)
final_fit = fit(final_workflow, trainingdrug)
...
```{r}
predtrain = predict(final_fit, trainingdrug)
confusionMatrix(predtrain$.pred_class,trainingdrug$Nicotine, positive="Yes")
...
Accuracy: 0.6949  
```{r}
predtest = predict(final_fit, testingdrug)
confusionMatrix(predtest$.pred_class,testingdrug$Nicotine, positive="Yes")
```
...
Answer: B. Overfitting is likely occurring